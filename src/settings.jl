        ...
        structures = begin
            Leaf = begin
                ...
                params = begin 
                    ...
                    # current reserves. 
                    k_E = 0.2, (0.0, 1.0), (:exposed, :time, :temp), "shoots reserve turnover rate"
                    k_EC = 0.2, (0.0, 1.0), (:exposed, :time, :temp), "shoots C-reserve turnover rate"
                    k_EN = 0.2, (0.0, 1.0), (:exposed, :time, :temp), "shoots N-reserve turnover rate"
                    ...
                    j_E_mai = 0.001, (0.0, 0.01), (:exposed, :time, :temp), "shoots spec somatic maint costs."
                    j_E_rep_mai = 0.001, (0.0, 0.01), (:exposed, :time, :temp), "shoots spec maturity maint costs "
                    # Production parameters
                    # - these parameters play no dynamic role but can dominate weights
                    j_P_mai = 0.01, (0.0, 0.1), (:exposed, :time, :temp), "shoot product formation linked to maintenance"

                    SLA = 9.10, (5.0, 30.0), "Ferns 17.4 Forbs 26.2 Graminoids 24.0 Shrubs 9.10 Trees 8.30"

                    K_autophagy = 0.000001, (0.0000001, 0.00001)
                    K_C = fraction_per_litre_gas_to_mols(40.0/1e6), "half-saturation concentration of carbon dioxide"
                    K_O = fraction_per_litre_gas_to_mols(0.0021), "half-saturation concentration of oxygen"
                    X_C = fraction_per_litre_gas_to_mols(400.0/1e6), "carbon dioxide @ 400ppm"
                    X_O = fraction_per_litre_gas_to_mols(0.21), "oxygen (21% volume in air) "
                    # Life stage parameters
                    M_Vgerm = 0.5, "shoots structural mass at germination"
                    M_Vrep = 10.0, "shoots structural mass at start reproduction" # TODO: isn't this variable/seasonally triggered?
                    ...
                    M_Vref = 4.0, (0.4, 20.0), :exposed
                    M_Vscaling = 400.0, (40.0, 2000.0), :exposed, "shoots scaling mass"
                    ...
                    y_V_E = 0.7, (:exposed, :time), "from shoots reserve to structure: 0.3 lost as CO2?"
                    y_P_V = 0.02, :exposed, "shoot product formation linked to growth: where does this carbon come from?"
                    y_E_CH_NO = 1.5, :exposed, "from shoots C-reserve to reserve, using nitrate: 0.75 EC per E."
                    y_E_EN = 0.5, :exposed, "from shoots N-reserve to reserve: 2 EN per E"
                    y_E_ET = 0.8, :exposed, "from shoots reserve to roots reserve:"
                    y_EN_ENT = 1.0, :exposed, "from roots N-reserve to shoots N-reserve"
                    ...
                    n_N_P = 0.0, "N/C in product (wood)"
                    n_N_V = 0.15, "N/C in structure. Shouldnt this be identical to the reserve?"
                    n_N_EC = 0.0, "N/C in C-reserve"
                    n_N_EN = 10.0, "N/C in N-reserve"
                    n_N_E = 0.2, "N/C in reserve. This should be calculated, not constant. 1.8181??? (10/11 * 0.3)/1.5 "

                    # Parameters that link moles to grams (wet weight)
                    # These should be calculated from first principles, not preset.
                    w_P = 25.0, "mol-weight of shoot product (wood): 1mol * 12g/mol = 12g"
                    w_V = 25.0, "mol-weight of shoot structure: 0.15mol * 14g/mol + 1mol * 12g/mol = 14.1g"
                    w_EC = 25.0, "mol-weight of shoot C-reserve: 1mol * 12g/mol = 12g"
                    w_EN = 25.0, "mol-weight of shoot N-reserve: 10mol * 14g/mol + 1mol * 12g/mol = 152g"
                    w_E = 25.0, "mol-weight of shoot reserve: 0.2mol * 14g/mol + 1mol * 12g/mol = 14.8g"

                    REFERENCE_TEMP = 310.0, (273.0, 325.0), "temp for which rate pars are given"
                    ARRH_TEMP = 2000.0, (200.0, 4000.0), "Arrhenius temp"
                    LOWER_BOUNDARY = 280.0, (273.0, 325.0), "lower boundary tolerance range temp_params[_UPPER_BOUNDARY] = 318.0 # K, upper boundary tolerance range"
                    ARRH_LOWER = 20000.0, (2000.0, 40000.0), "Arrhenius temp for lower boundary"
                    UPPER_BOUNDARY = 315.0, (273.0, 325.0), "upper boundary tolerance range"
                    ARRH_UPPER = 70000.0, (7000.0, 140000.0), "Arrhenius temp for upper boundary"
            ...
            Root = begin
                ...
                params = begin
                    j_NH_Amax = μmol_to_mol(50.0,), μmol_to_mol.((0.1, 1000.0)), "max spec uptake of ammonia"
                    j_NO_Amax = μmol_to_mol(50.0,), μmol_to_mol.((0.1, 1000.0)), "max spec uptake of nitrate"
                    j_E_rep_mai = 0.0, "roots spec maturity maint costs "

                    # Water affects root nutrient uptake via the saturation parameters
                    K_NH = mmol_to_mol(10.0), "half-saturation concentration of ammonia"
                    K_NO = mmol_to_mol(10.0), "half-saturation concentration of nitrate"
                    K_H = 10.0, "half-saturation concentration of water"
                    X_NH = mmol_to_mol(5.0), "ammonia"
                    X_NO = mmol_to_mol(10.0), "concentration of nitrate see e.g. [_@crawford1998molecular]"
                    X_H = 10.0 

                    # Life stage parameters
                    M_Vgerm = 0.3, "roots structural mass at germination"
                    M_Vrep = 10.0, "roots structural mass at start reproduction"
                    ...
                    y_E_CH_NO = 1.5, "from roots C-reserve to reserve, using nitrate"
                    y_E_CH_NH = 1.25, "from roots C-reserve to reserve, using ammonia"
                    y_EC_ECT = 1.0, "from shoots C-reserve to roots C-reserve"
                    y_E_EN = 0.3, "from roots N-reserve to reserve: why is this different to shoots?"
                end
            end
        end
    end
