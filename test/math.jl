...
@testset "Math" begin
    ...
    global (a, b, c) = stoich_merge(2.0, 2.0, 1.0, 1.0)
...
@testset "Rate" begin
    ...
    global rf         = rate_formula(rate, mass, A_turnover, j_E_mai, y_E_CH_NO, y_E_EN, y_V_E, kap_soma)
    ...
    @testset "test rate with units" begin
        ...
        global rate = 0.001u"mol/mol*d^-1"
        @test_nowarn catabolic_flux(mass, A_turnover, rate)
        global rf = rate_formula(rate, mass, A_turnover, j_E_mai, y_E_CH_NO, y_E_EN, y_V_E, kap_soma)
