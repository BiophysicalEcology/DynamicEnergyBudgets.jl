...
function test_settings(tspan::Tspan; save_intermediate=true, use_environment=false, 
                       state_type=StatePVCNE, environment = DataFrame(), years=1, 
                       timestep_days=1.0/(24.0*60*60), location=[148, -32]) # Everything was per day despite units in seconds and days.

    load_settings(DEBSettings, TestPars, OrderedDict(
