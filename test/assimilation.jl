using DynamicEnergyBudgets, DimensionalData

using DynamicEnergyBudgets: photosynthesis, nitrogen_uptake
...
function nfactory()
    ...
    p2 = o2.params;
    u1 = DimensionalArray([9.0,8.0,7.0]u"mol", X(Val(DynamicEnergyBudgets.STATE)))
    u2 = DimensionalArray([9.0,8.0,7.0]u"mol", X(Val(DynamicEnergyBudgets.STATE)))
    u2 .*= 2.7
    du1 = DimensionalArray(fill(0.0u"mol*hr^-1", 3), X(Val(DynamicEnergyBudgets.STATE)))
    du2 = DimensionalArray(fill(0.0u"mol*hr^-1", 3), X(Val(DynamicEnergyBudgets.STATE)))
...
@testset "N assimilation" begin
    ...
    @testset "N assimilation flux is added correctly" begin
        @test nitrogen_uptake(f, o1, u1) > 0.0u"μmol/s"
        ...
        c2 = sum(du2)
        # @test n1 == o1.J[:N,:ass]
        # @test n1 == nitrogen_uptake(f, o1, u1)
    end
    ...
    @testset "N assimilation flux is merged correctly" begin
        ...
        o1a, p1a, u1a, du1a, o2a, p2a, u2a, du2a, fa = nfactory();
        set_rej!(o1a, o2a)
        sum_flux!(du1a, o1a, 0); sum_flux!(du2a, o2a, 0)

        o1b, p1b, u1b, du1b, o2b, p2b, u2b, du2b, fa = nfactory();
        set_rej!(o1b, o2b)
        ...
        uptake_n = nitrogen_uptake(f, o1b, u1b)
        ...
        c1a, c2a, c1b, c2b = sum.((du1a, du2a, du1b, du2b))
        n1a, n2a, n1b, n2b = dot.((du1a, du2a, du1b, du2b), (n_ratios,))

        c_loss = o1b.J1[:C,:los] + o2b.J1[:C,:los]
        n_loss = o1b.J1[:N,:los] + o2b.J1[:N,:los]
...
@testset "C assimilation" begin
    ...
    @testset "C assimilation flux is merged correctly" begin
        ...
        o1a, p1a, u1a, du1a, o2a, p2a, u2a, du2a, fa = cfactory();
        ...
        o1b, p1b, u1b, du1b, o2b, p2b, u2b, du2b, fa = cfactory();
        sum_flux!(du1b, o1b, 0); sum_flux!(du2b, o2b, 0)
        ...
        uptake_c = photosynthesis(f, o1b, u1b)
